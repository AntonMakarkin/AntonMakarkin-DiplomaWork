...
const updateOrderByIdByCourier = () => {
    return async (req, res) => {
        const { id } = req.params;
        ...
        const item = await Order.find({ _id: id, "courier._id": convertedCourierId }, '-courier')
        ...
        if (!isValidOperation) {
            return res.status(400).send({ error: 'Невозможно обновить данные параметры заказа!' });
        }

        try {
            updates.forEach((update) => itemOne[update] = req.body[update]); //updating the item
            await itemOne.save();
            res.json(itemOne);
        } catch (err) {
            res.status(500).send({ error: err.message });
...
module.exports = {
    ...
    getOrderByIdForCourier, updateOrderById, updateOrderByIdByCourier
}